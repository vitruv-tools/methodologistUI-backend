...
public class KeycloakApiHandler {
    ...
    public KeycloakWebToken getExchangeTokenOrThrow(String token, String userId) {
        ...
        try {
            Map<String, String> fieldMap = mapper.convertValue(new ExchangeTokenPostBody(token, userId), new TypeReference<Map<String, String>>() {});
    ...
    public KeycloakWebToken getAccessTokenByRefreshToken(String refreshToken) {
        ...
        return webClient.post()
                .uri(POST_TOKEN_URL)  // Example: http://localhost:8383/realms/investino/protocol/openid-connect/token
                .contentType(MediaType.APPLICATION_FORM_URLENCODED)
                .body(BodyInserters.fromFormData(formData))
                .retrieve()
                .onStatus(HttpStatusCode::is4xxClientError, response -> {
                    ...
                    return Mono.error(new UncaughtRuntimeException("Client error: " + response.statusCode()));
                })
