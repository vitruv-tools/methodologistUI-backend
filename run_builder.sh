...
# usage: ./run_builder.sh /abs/path/model.ecore /abs/path/model.genmodel [/tmp/mm-out]
ECORE="${1:?ecore missing}"
GENMODEL="${2:?genmodel missing}"
OUTDIR="${3:-/tmp/mm-out}"

JAR="/opt/methodologist/methodologist-build.jar"
IMAGE="eclipse-temurin:21-jre"

JOB="$(mktemp -d /tmp/mm-job-XXXXXX)"
mkdir -p "$JOB/input" "$JOB/output"

# copy inputs into the job folder (NO placeholders)
...
docker run --rm --read-only --network none --cpus 1 --memory 1g \
  ...
  -v "$JOB":/work:rw "$IMAGE" \
  java -jar /work/methodologist-build.jar \
